...
class Secure_bcrypt extends CI_Controller
{
  ...
  public function token()
  {
    ...
    $this->db_session->set_userdata('token',$token);
    return $token;
  }
  ...
  public function register_bcrypt()
  {
    ...
    if($this->input->post('token') && $this->input->post('token') == $this->db_session->userdata('token'))
    {
    ...
    if($this->input->post('token') && $this->input->post('token') == $this->db_session->userdata('token'))
    {
      ...
      //en otro caso procesamos los datos
        ...
        if($login)
        {
          ...
          $this->db_session->set_userdata($data);
          $this->login_ok();
        }
      }
    }
