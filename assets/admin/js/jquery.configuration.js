...
{
    if(confirm('¿Está seguro que desea eliminar éste ítem?'))
    {
            $("#filter").attr("action", pagina)
            $("#filter").submit();
    }
...
{
    fu = document.getElementById(inputFile);

    if(!extensionVerif(fu.value))
    {
        alert("Solo se permiten archivos tipo JPG, PNG ó GIF");
        fu.value = "";
    }
}